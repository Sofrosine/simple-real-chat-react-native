...
class EditProfile extends Component {
  ...
  insertData = async () => {
    const {name, dob, gender, number, latitude, longitude, uri} = this.state;
    const uid = firebase.auth().currentUser.uid;
  ...
  async componentDidMount() {
    ...
    if (granted) {
      console.log('GRAHANANANJSAKJDKAJDKLAJSDK')
      Geolocation.getCurrentPosition(
        position => {
          console.log('My current location', JSON.stringify(position));
          console.log('cawewq', position)
          this.setState({
            latitude: position.coords.latitude.toString(),
            longitude: position.coords.longitude.toString(),
          });
          console.log('ajsdkjasdkjqwhiejhqiowe',position)
          console.log('latittud', this.state.latitude)
        },
        error => {
        ...
        <View
          ...
          <View
            ...
            <View style={{width: '50%'}}>
              ...
              <DatePicker
                style={{width: 150,padding: 5,borderColor: 'pink',borderWidth:1,}}
                date={this.state.dob}
                mode="date"
          ...
          <View style={{alignSelf: 'flex-start', width: '100%',marginBottom:10}}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
              style={{ width: 200, backgroundColor: 'pink', height: 45, borderRadius: 50, alignItems: 'center',
               justifyContent: 'center' }}>
