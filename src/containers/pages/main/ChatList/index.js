  ...
  handleChat = async (friendUID) => {
    ...
    ref.on('value',async snapshot => {
      if(Object.keys(snapshot.val()) != null) {
        await Object.keys(snapshot.val()).map(async key => {
          const ref2 = firebase.database().ref(`/chat/${friendUID}/${uid}/${key}/isRead`)
          ref2.remove()
              ...
              <TouchableOpacity
                ...
                <View>
                  ...
                  <View style={{flexDirection:'row'}}>
                    ...
                    <View style={{height:8, width:8, borderRadius:8, backgroundColor:'gray',opacity:0.5,alignSelf:'center',marginRight:'5%',marginTop:'5%'}}/>
                  : <View style={{height:8, width:8, borderRadius:8, backgroundColor:'pink',alignSelf:'center',marginRight:'5%',marginTop:'5%'}}/>}
                    <Text style={{fontSize: 12, color: 'gray',alignSelf:'center'}}>
                      {data.data.status}
                    </Text>
                  </View>
                </View>
                      {this.state.data2[`${data.data.uid}`] != null ? 
                  <View style={{ height: 20, width: 20, backgroundColor: 'pink', borderRadius: 10, position: 'absolute', right: '3%', justifyContent: 'center', alignItems: 'center' }}>
                    <Text style={{color:'white'}}>{this.state.data2[`${data.data.uid}`]}</Text>
                  </View>
                      : null}
